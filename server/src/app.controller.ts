import { Get, Delete, Query, Put, Body, Controller } from '@nestjs/common'
...
export class AppController {
  ...
  async register(@Body() data: any) {
    let device = await this.deviceService.find(data.id)
    ...
    device.expiresAt = new Date().setDate(new Date().getDate() + 1)
    return await this.deviceService.saveDevice(device)
  }

  @Get('/heartbeat')
  async heartbeat(@Query('id') id: string) {
    let device = await this.deviceService.find(id)
    if (device) {
      device.lastHeartbeat = new Date()
      device.expiresAt = new Date().setDate(new Date().getDate() + 1)
      return await this.deviceService.saveDevice(device)
